const fs = require('fs');
const path = require('path');

// Function to create a default image
function createDefaultImage() {
  const publicDir = path.join(__dirname, '../../public');
  const imagesDir = path.join(publicDir, 'images');
  const defaultImagePath = path.join(imagesDir, 'default-course-thumbnail.jpg');
  
  console.log('Creating default image structure:');
  
  // Create public directory if it doesn't exist
  if (!fs.existsSync(publicDir)) {
    console.log(`Creating directory: ${publicDir}`);
    fs.mkdirSync(publicDir);
  }
  
  // Create images directory if it doesn't exist
  if (!fs.existsSync(imagesDir)) {
    console.log(`Creating directory: ${imagesDir}`);
    fs.mkdirSync(imagesDir);
  }
  
  // Only create the default image if it doesn't exist
  if (!fs.existsSync(defaultImagePath)) {
    console.log(`Creating default image: ${defaultImagePath}`);
    
    // Create a simple colored square (1x1 pixel) JPEG
    // This is a minimal JPEG file format for a blue square
    const minimalJpegBytes = Buffer.from([
      
    ]);
    
    fs.writeFileSync(defaultImagePath, minimalJpegBytes);
    console.log('Default image created successfully!');
  } else {
    console.log(`Default image already exists at: ${defaultImagePath}`);
  }
}

createDefaultImage();
console.log('\nRun this script to create a simple default image that can be used when other images fail to load.');

module.exports = { createDefaultImage };
